...
export function SectionDeliverables({ data, index = 0 }: { data: SectionDeliverablesData; index?: number }) {
  const columns = [
    { label: data.includedLabel || 'Typically included', items: data.included ?? [], mark: 'before:bg-pop' },
  ...
  return (
    <SectionShell id={data.id} eyebrow={data.eyebrow} heading={data.heading} index={index}>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {columns.map((col) => (
          <div key={col.label} className="surface-card flex flex-col gap-4 p-6 md:p-8">
            <h3 className="text-label uppercase text-muted">{col.label}</h3>
            <ul className="flex flex-col gap-2.5 text-text">
              {col.items.map((item) => (
                <li
                  key={item}
                  className={`relative pl-6 before:absolute before:left-0 before:top-[0.7em] before:h-[3px] before:w-3.5 before:rounded-full ${col.mark}`}
                >
                  {item}
                </li>
              ))}
